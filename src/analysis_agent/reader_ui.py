...
def refine(page, entries, flights_payload):
    ...
    kpis = soup.select_one(".kpi-grid")
    card = soup.select_one(".narrative-card")
    ...
    for kpi in kpis.select(".kpi-card"):
        if kpi.get("data-kpi") in {"load_factor_reported", "passengers"}:
            kpi.decompose()
    margin = BeautifulSoup(
        """<article class="kpi-card" data-kpi="unit_margin_cents_per_km" data-accent="green"><span class="kpi-info" title="Diferencia entre RASK y CASK por asiento-kilómetro ofrecido." aria-label="Diferencia entre RASK y CASK por asiento-kilómetro ofrecido.">i</span><p class="kpi-label">Margen unitario</p><p class="kpi-value" id="kpi-unit_margin_cents_per_km-value">—</p><div class="kpi-comparisons"><div class="comparison-chip"><span>vs. trimestre anterior</span><strong id="kpi-unit_margin_cents_per_km-qoq">—</strong></div><div class="comparison-chip"><span>vs. año anterior</span><strong id="kpi-unit_margin_cents_per_km-yoy">—</strong></div></div></article>""",
        "html.parser",
    )
    kpis.append(margin.article)
    tab_buttons = "".join(
        f'<button id="tab-{spec.key}" role="tab" aria-selected="{str(index == 0).lower()}" aria-controls="panel-{spec.key}"'
        + ("" if index == 0 else ' tabindex="-1"')
        + f">{spec.title}</button>"
        for index, spec in enumerate(READER_TAB_SPECS)
    )
    tab_panels = "".join(
        f'<section id="panel-{spec.key}" role="tabpanel" aria-labelledby="tab-{spec.key}"'
        + ("" if index == 0 else " hidden")
        + "></section>"
        for index, spec in enumerate(READER_TAB_SPECS)
    )
    tabs = BeautifulSoup(
        f'<div class="reader-tabs" role="tablist" aria-label="Vistas del dashboard">{tab_buttons}</div>{tab_panels}',
        "html.parser",
    )
    kpis.insert_before(tabs)
    reading = soup.find(id="panel-reading")
    economy = soup.find(id="panel-economy")
    reading.append(card.extract())
    unit = soup.find(id="unit-heading").find_parent("section")
    economy.append(kpis.extract())
    for element in [unit, soup.select_one(".chart-grid"), soup.select_one("details.disclosure")]:
        economy.append(element.extract())
    flights = soup.find(id="panel-flights")
    flights.append(BeautifulSoup(render_flights_panel(flights_payload), "html.parser"))
    for button in soup.select('[data-analysis-open$="evidence"]'):
        button.decompose()
    for dialog in soup.select('dialog[id$="evidence"]'):
        dialog.decompose()
    for record, authorized, package, calculations, checks in entries:
        ...
        claims = {c["claim_id"]: c for c in record["draft"]["claims"]}
        numbers = {}
        ...
        def cite(container, claim):
            for binding in claim.get("bindings", {}).values():
                ...
                for text in list(container.find_all(string=True)):
                    if value not in str(text) or text.parent.name in ("a", "sup"):
                        continue
                    ...
                    a = soup.new_tag(
                        ...
                        attrs={"class": "source-note", "aria-label": f"Fuente {number}: abrir dato original"},
                    )
                    a.string = str(number)
                    a["title"] = (
                        "Reporte original · "
                    ...
                    sup.append(a)
                    text.replace_with(NavigableString(before + value), sup, NavigableString(after))
                    break

        period = authorized["period_id"]
        block = soup.select_one(f'[data-analysis-period="{period[-1]}T{period[2:4]}"]')
        for li, cid in zip(block.select(".analysis-summary li"), record["draft"]["summary_claim_ids"]):
            cite(li, claims[cid])
    ...
    flight_data = soup.new_tag("script", id="flight-dashboard-data", type="application/json")
    flight_data.string = (
        json.dumps(flights_payload, ensure_ascii=False, separators=(",", ":"), allow_nan=False)
        .replace("&", "\\u0026")
        .replace("<", "\\u003c")
        .replace(">", "\\u003e")
        .replace("\u2028", "\\u2028")
