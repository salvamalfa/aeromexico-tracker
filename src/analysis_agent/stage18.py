...
def consumer_html(dashboard, entries, flights=None):
    ...
    def clean(value):
        if isinstance(value,dict):
            for key in list(value):
                if key in {'narrative','narrative_text','executive_reading'}: del value[key]
                else: clean(value[key])
        elif isinstance(value,list):
            for item in value: clean(item)
    clean(payload)
    page=render_executive_html(payload)
    blocks=[];dialogs=[];manifest=[];seen=set()
    for record,authorized,package,calculations,checks in entries:
        period=authorized['period_id']
        if period in seen: raise ValueError('Multiple versions for one period')
        seen.add(period)
        prefix='analysis-'+period+'-';label=f'{period[-1]}T{period[2:4]}'
        items=[]
        for item in authorized['summary_items']:
            lead=item.get('lead') or '';text=item['text']
            content=emphasized(text,lead,item.get('emphasis',[]))
            items.append('<li>'+content+'</li>')
        blocks.append(f'<div data-analysis-period="{label}" hidden><h3>{escape(authorized["thesis"])}</h3><ul class="analysis-summary">'+''.join(items)+f'</ul><div class="analysis-actions"><button data-analysis-open="{prefix}full">▸ Leer análisis completo</button><button data-analysis-open="{prefix}evidence">▸ Ver evidencia</button></div></div>')
        support=BeautifulSoup(render_review(record,checks,package,calculations),'html.parser')
        context=''.join('<aside class="analysis-context">'+escape(t)+'</aside>' for t in authorized.get('context',[]))
        blocks[-1]=blocks[-1].replace('<div class="analysis-actions">',context+'<div class="analysis-actions">')
        for name in ('full','evidence'):
            dialog=support.find('dialog',id=name)
            dialog['class']=['analysis-dialog'];dialog['data-prefix']=prefix
            for el in [dialog,*dialog.find_all(id=True)]: el['id']=prefix+el['id']
            dialog['aria-labelledby']=prefix+dialog['aria-labelledby']
            if name=='full': dialog.select_one('.modal-content > .muted').string='Análisis aprobado · cifras del expediente al corte'
            dialogs.append(str(dialog))
        manifest.append({k:authorized[k] for k in ('period_id','version','content_hash','evidence_fingerprint','approval_event','audit_hash')})
    ...
    if page.count(old)!=1: raise ValueError('Dashboard integration anchor changed')
    page=page.replace(old,'<div id="narrative-copy">'+''.join(blocks)+'<p id="analysis-empty">Análisis pendiente de aprobación para este trimestre.</p></div>')
    ...
    return refine(page.replace('</body>',''.join(dialogs)+'<script type="application/json" id="analysis-manifest">'+encoded+'</script><script>'+SCRIPT+'</script></body>'),entries,flights)
...
def publish(records, output, dashboard=None, root=flow.ROOT):
    ...
    """
    output=Path(output).resolve();root=Path(root)
    ...
    with flow.writer(root):
        ...
        receipt={'html_sha256':fingerprint,'output':str(output),'versions':[r['version'] for r in records],
                 'approval_events':[e[1]['approval_event'] for e in entries]}
        key=flow.digest(receipt);store=root/'publications';store.mkdir(parents=True,exist_ok=True)
        intent=store/(key+'.intent.json');done=store/(key+'.published.json')
        if not intent.exists(): flow.atomic_json(intent,receipt)
        ...
        if hashlib.sha256(output.read_bytes()).hexdigest()!=fingerprint: raise ValueError('Published file hash mismatch')
        if not done.exists(): flow.atomic_json(done,{**receipt,'published_at':datetime.now(timezone.utc).isoformat()})
        return receipt
