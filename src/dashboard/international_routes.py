...
def _item_capacity_metrics(cell, passengers, passengers_low, passengers_high):
    ...
    return dict(
        ...
        seats=seats,
        seats_low=cell["seats_low"],
        seats_high=cell["seats_high"],
...
def _route_capacity_metrics(items):
    ...
    if not complete:
        return dict(
            ...
            seats_high=None,
            load_factor=None,
            load_factor_low=None,
            load_factor_high=None,
            capacity_estimated=False,
            load_factor_status="capacity_incomplete",
            aircraft_model_coverage=None,
        )
...
def _estimated_routes(estimates, capacity, months, endpoint):
    ...
    for market, group in frame.groupby("market_key"):
        ...
        if "US" in (endpoint(a)["country"], endpoint(b)["country"]):
            continue
        monthly = []
        for (period, origin, dest), part in group.groupby(
            ...
            item = dict(
                ...
                passengers_low=passengers_low,
                passengers_high=passengers_high,
                support_observed_in_period=True,
                support_source_periods=str(period),
                support_month_gap=0,
            )
...
def extend_networks(connection, networks):
    exists = connection.execute(
        "SELECT count(*) FROM information_schema.tables WHERE table_name='fact_international_route_observations'"
    ).fetchone()[0]
    if not exists:
        return deepcopy(networks)
    ...
    aena = frame[
        (frame.source_system == "aena")
        & (frame.operator_icao == "AMX")
        & (frame.observation_scope == "carrier_airport")
    ].copy()
    has_aicm = connection.execute(
        "SELECT count(*) FROM information_schema.tables WHERE table_name='fact_aicm_international_scheduled_route_movements'"
    ).fetchone()[0]
    scheduled = (
        connection.execute(
            "SELECT * FROM fact_aicm_international_scheduled_route_movements WHERE observation_status='assigned_slot_not_flown'"
        ).df()
        if has_aicm
        else pd.DataFrame()
    )
    has_oma = connection.execute(
        "SELECT count(*) FROM information_schema.tables WHERE table_name='fact_oma_documented_routes'"
    ).fetchone()[0]
    ...
    estimates = (
        connection.execute(f"SELECT * FROM {ESTIMATE_TABLE} WHERE carrier_key IN {ESTIMATED_CARRIERS}").df()
        if has_estimate
        else pd.DataFrame()
    )
    has_capacity = connection.execute(
        f"SELECT count(*) FROM information_schema.tables WHERE table_name='{CAPACITY_TABLE}'"
    ).fetchone()[0]
    capacity = (
        connection.execute(f"SELECT * FROM {CAPACITY_TABLE} WHERE carrier_key IN {ESTIMATED_CARRIERS}").df()
        if has_capacity
        else pd.DataFrame()
    )
    ...
    def total(group, key):
        if group.empty or group[key].isna().any():
            return None
    ...
    for period, network in result.items():
        ...
        route_frame["market_key"] = route_frame.apply(
            lambda r: "<>".join(sorted([r.origin_iata, r.dest_iata])), axis=1
        )
        for (source, market), group in route_frame[route_frame.period_id.isin(months)].groupby(
            ["source_system", "market_key"]
        ):
            if any(r["market_key"] == market for r in network["routes"]):
                raise ValueError("Overlapping authority market requires precedence review")
            ...
            prior = route_frame[
                (route_frame.source_system == source)
                & (route_frame.market_key == market)
                & route_frame.period_id.isin(previous_months)
            ]
            ...
            if len(directions_set) < 2:
                continue

            def both_directions(g, month):
                return (
                    set(zip(g[g.period_id == month].origin_iata, g[g.period_id == month].dest_iata))
                    == directions_set
                )
            ...
            comparable = (
                complete
                and set(prior.period_id) == set(previous_months)
                and all(both_directions(prior, m) for m in previous_months)
            )
            a, b = market.split("<>")
            network["routes"].append(
                dict(
                    ...
                    destination=endpoint(b),
                    **{
                        k: total(group, k) if complete else None
                        for k in ("passengers", "seats", "departures")
                    },
                    load_factor=None,
                    previous={
                        k: total(prior, k) if comparable else None
                        for k in ("passengers", "seats", "departures")
                    },
                    ...
                    coverage_note="Meses: "
                    + ", ".join(m[5:] for m in observed)
                    + " · "
                    + LABELS[source]
                    + (" · falta un sentido" if not complete else ""),
                ...
                for market, group in current_slots.groupby("market_key"):
                    ...
                    network["routes"].append(
                        dict(
                            ...
                            passengers=None,
                            seats=None,
                            departures=scheduled_count,
                            load_factor=None,
                            previous={k: None for k in ("passengers", "seats", "departures")},
                            directions=directions,
                            source_label=AICM_LABEL,
                            observed_months=[],
                            scheduled_months=sorted(group.period_id.unique()),
                ...
                for market, group in quarter_routes.groupby("market_key"):
                    ...
                    directions = [
                        dict(
                            ...
                            passengers=None,
                            seats=None,
                            departures=int(row.scheduled_movements)
                            if pd.notna(row.scheduled_movements)
                            else None,
                        )
                        for row in group.itertuples(index=False)
                    ]
                    count = (
                        int(group.scheduled_movements.sum())
                        if group.scheduled_movements.notna().all()
                        else None
                    )
                    status = (
                        "scheduled_from_dated_release"
                        if count is not None
                        else "documented_operating_route_count_unavailable"
                    )
                    network["routes"].append(
                        dict(
                            market_key=market,
                            ...
                            previous={k: None for k in ("passengers", "seats", "departures")},
                            directions=directions,
                            source_label=OMA_LABEL,
                            observed_months=[],
                            scheduled_months=months if count is not None else [months[0]],
                            coverage_note=(
                                f"Abr–jun 2026 · OMA · {count} programados"
                                if count is not None
                            ...
                            carrier_role="airline_named_in_airport_release",
                            marketing_carrier=None,
                            agent_eligible=False,
                        )
                    )
        ...
        for market, estimate in _estimated_routes(estimates, capacity, months, endpoint).items():
            ...
            if matching is not None:
                ...
                own_months = set(
                    matching.get("observed_months") or matching.get("scheduled_months") or months
                )
                ...
                for item in monthly:
                    ...
                    sums = by_direction.setdefault(
                        key, dict(passengers=0.0, passengers_low=0.0, passengers_high=0.0)
                    )
                    for metric in sums:
                ...
                for direction in matching["directions"]:
                    ...
                    direction.update(
                        by_direction.get(key, dict(passengers=0.0, passengers_low=0.0, passengers_high=0.0))
                    )
                ...
                matching["coverage_note"] += (
                    " · Pasajeros estimados de Grupo Aeroméxico (AFAC + AeroDataBox) · meses "
                    + ", ".join(sorted({m["period_id"][5:] for m in monthly}))
                )
                continue
            network["routes"].append(
                dict(
        ...
        for airport, group in current.groupby("airport_iata"):
            ...
            for metric in ("passengers", "operations"):
                values[metric] = total(
                    group[(group.metric_key == metric) & group.period_id.isin(shared)], metric
                )
            activity.append(
                dict(
                    airport_iata=airport,
                    airport_name=endpoint(airport)["name"],
                    passengers=values["passengers"],
                    operations=values["operations"],
                    observed_months=shared,
                    expected_months=months,
                    source_label="España · Aena",
        ...
        network["coverage"] = (
            "Mercados observados y programados por fuente; cobertura parcial de la red internacional"
        )
            ...
            label: sorted(
                {
                    ...
                    for m in (
                        r.get("observed_months") or r.get("scheduled_months") or r.get("estimated_months", [])
                    )
                }
            )
            for label in sorted({r["source_label"] for r in network["routes"]})
        }
