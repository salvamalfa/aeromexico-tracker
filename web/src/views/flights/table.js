...
import { $, deltaDisplay, esc, finite, formatRouteMetric, integer } from "./dom.js";
import { DOMESTIC_MONTH_NAMES, ESTIMATE_INFO_TITLE, state } from "./state.js";
import { coverageDotHtml, scheduledIconHtml, sourceFooterHtml } from "./coverage.js";
import { bindAirportSearchControls } from "./search.js";
import { routeTitle } from "./network.js";
...
export function airportRouteColor(index) {
  return routePalette[index] || `hsl(${Math.round((index * 137.508 + 18) % 360)} 68% 40%)`;
}

export function renderRouteDetailPlaceholder() {
...
export function routeMetricChangeChip(route, key) {
...
function renderDomesticRouteTable(title, subtitle, tableRoutes, subtitleHtml) {
  $("airport-tooltip").innerHTML = `
...
function directionValue(direction, key, route) {
...
function estimateDirectionLine(item) {
...
// meses.
function estimateDetails(route) {
...
function directionDetails(route) {
...
export function renderRouteTable(title, subtitle, tableRoutes, subtitleHtml = null) {
...
export function renderAirportTooltip(airport, incident) {
