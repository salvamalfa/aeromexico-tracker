...
export interface FlightsState {
  dataRoot: string;
  metadata: FlightsMetadata | null;
  quarters: QuarterRecord[];
  byPeriod: Map<string, QuarterRecord>;
  monthlyPassengers: MonthlyPassengerSeries | null;
  ...
  passengerPeriod: "quarter" | "month";
  selectedMarket: string | null;
  selectedRegion: string | null;
  pinnedAirport: string | null;
  hoveredAirport: string | null;
  hoveredAirportAt: number;
  flowMapRendered: boolean;
  selectAirportFromSearch: ((iata: string) => void) | null;
  ...
  internationalNetworks: Map<string, PeriodNetworkDocument>;
}
...
export function initState(quartersDoc: QuartersDocument): void {
  ...
  state.periodIndex = Math.max(
    0,
    state.quarters.findIndex((item) => item.period_id === state.metadata?.default_period)
  );
...
export function monthsInQuarter(quarterId: string | null): string[] {
  if (!quarterId) return [];
  return state.availablePeriods.domestic_monthly.filter(
    (periodId) =>
      `${periodId.slice(0, 4)}Q${Math.floor((Number(periodId.slice(5, 7)) - 1) / 3) + 1}` === quarterId
  );
}

export function domesticAvailableForQuarter(quarterId: string): boolean {
...
export async function loadQuarters(dataRoot?: string): Promise<void> {
  ...
  const quartersDoc = await fetchJson<QuartersDocument>(`${state.dataRoot}/flights/quarters.json`);
