...
export function renderMix(record) {
  ...
  const points = state.passengerPeriod === "quarter" ? [...quarterGroups.values()] : monthlyPoints;
  ...
  const hoverData = points.map((point) => {
    const heading = state.passengerPeriod === "quarter"
      ? point.period_label
